...
| `docker/` | `Dockerfile.<stack>`, `dockerignore.<stack>`（フレームワークごと） | `templates/*/.devcontainer/Dockerfile`, `templates/rails/.dockerignore` など | `generate-devcontainer.ts`, `gen-common-files.ts` |
...
- **フレームワーク別** — next.config.ts（Next.js）、vite.config.ts（React）、.golangci.yml（Go）、rust-toolchain.toml（Rust）、pint.json / phpunit.xml（Laravel）、.csproj / global.json / .sln（C#）は `templates/<stack>/` 直下を正本として直接編集する（生成しない）。`.dockerignore` は `shared/docker/dockerignore.<stack>` を正本とし、フレームワークごとに1ファイル用意する。
