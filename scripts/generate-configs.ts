...
const RSPEC_COMMON = join(ROOT, "shared", "rspec", "rspec.common");
const SHARED_ESLINT = join(ROOT, "shared", "eslint");
...
for (const dir of SHARED_CONFIG_STACKS) {
  ...
  const GEN_HEADER_ESLINT = "// Generated by scripts/generate-configs.ts — edit shared/eslint/eslint.config.<stack>.ts instead.\n\n";
  const nodeEslintContent = readFileSync(join(SHARED_ESLINT, "eslint.config.base.ts"), "utf8");
  ...
  // ── .rubocop.yml（shared/rubocop のベース + テンプレート用 fragment をマージ）────
