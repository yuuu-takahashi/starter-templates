...
describe('generate-root-workflow', () => {
  describe('transformStepsForMonorepo', () => {
    it('checkout ステップが除外され、先頭に追加される', () => {
      ...
      const result = transformStepsForMonorepo(steps, 'minimal-templates/nextjs');
    ...
    it('with.path に ${dir}/ プレフィックスが付与される（相対パス）', () => {
      ...
      expect(setupStep?.with?.path).toBe(
  ...
  describe('mergeFormatAndLintSteps', () => {
    it('基本的なステップが生成される', () => {
      const result = mergeFormatAndLintSteps(undefined, undefined, 'minimal-templates/nextjs');
      expect(result[0].uses).toBe('actions/checkout@v4');
    ...
    it('yarn lint がある場合はコマンドに含まれる', () => {
      ...
      const formatStep = result.find(
        (s) => s.name === 'yarn format & lint'
      );
