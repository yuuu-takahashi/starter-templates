...
describe('stacks.ts 整合性', () => {
  ...
  it('ROOT_STACKS の dir が全て TEMPLATE_DIRS に含まれる', () => {
    for (const entry of ROOT_STACKS) {
      expect(
        TEMPLATE_DIRS,
        `ROOT_STACKS の dir="${entry.dir}" が TEMPLATE_DIRS に未登録`,
      ).toContain(entry.dir);
    }
  });
  ...
  it('NODE_VERSION_DIRS / RUBY_VERSION_DIRS は TEMPLATE_DIRS のサブセット', () => {
    const all = new Set(TEMPLATE_DIRS);
    for (const dir of [...NODE_VERSION_DIRS, ...RUBY_VERSION_DIRS]) {
  ...
  it('MONOREPO_PREFIX_STACKS の各 slug は TEMPLATE_DIRS の dir から導出できる', () => {
    ...
    for (const slug of MONOREPO_PREFIX_STACKS) {
      expect(
        dirSlugs,
        `MONOREPO_PREFIX_STACKS の ${slug} が TEMPLATE_DIRS に存在しない`,
      ).toContain(slug);
    }
  });
