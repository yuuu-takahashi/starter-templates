...
import { ROOT_STACKS, MONOREPO_PREFIX_STACKS, TEST_SOURCE, STACK_DEFINITIONS } from "./lib/stacks.js";
...
const buildTestWorkflow = (): Record<string, WorkflowJob> => {
  ...
  const filtersBlock = testStacks.map((s) => `${s.id}:\n  - '${s.pathFilter}'`).join("\n");

  jobs.changes = {
    "runs-on": "ubuntu-latest",
    permissions: { contents: "read", "pull-requests": "read" },
    outputs: Object.fromEntries(testStacks.map((s) => [s.id, "${{ steps.filter.outputs." + s.id + " }}"])),
    steps: [
      { uses: "actions/checkout@v4" },
      { uses: "dorny/paths-filter@v3", id: "filter", with: { filters: filtersBlock } },
    ],
...
const buildDevcontainerWorkflow = (): Record<string, WorkflowJob> => {
  ...
  const filtersBlock = STACK_DEFINITIONS.map((s) => `${s.id}:\n  - '${s.dir}/.devcontainer/**'`).join("\n");

  jobs.changes = {
    "runs-on": "ubuntu-latest",
    permissions: { contents: "read", "pull-requests": "read" },
    outputs: Object.fromEntries(STACK_DEFINITIONS.map((s) => [s.id, "${{ steps.filter.outputs." + s.id + " }}"])),
    steps: [
      { uses: "actions/checkout@v4" },
      { uses: "dorny/paths-filter@v3", id: "filter", with: { filters: filtersBlock } },
    ],
...
const header = "# Generated by yarn generate:ci - edit shared/workflows/ or template's .github/workflows/ instead.\n\n";

// Generate code-check.yml
const codeCheckTemplatePath = join(ROOT, "scripts", "templates", "code-check.yml");
const codeCheckTemplate = readFileSync(codeCheckTemplatePath, "utf8").trimEnd();
const codeCheckJobs = buildRootWorkflow();
const codeCheckJobsYaml = YAML.stringify({ jobs: codeCheckJobs }, { lineWidth: 0 });
const codeCheckOutPath = join(ROOT, ".github", "workflows", "code-check.yml");
writeFileSync(codeCheckOutPath, header + codeCheckTemplate + "\n" + codeCheckJobsYaml, "utf8");
...
const devcontainerTemplatePath = join(ROOT, "scripts", "templates", "devcontainer-build.yml");
const devcontainerTemplate = readFileSync(devcontainerTemplatePath, "utf8").trimEnd();
const devcontainerJobs = buildDevcontainerWorkflow();
const devcontainerJobsYaml = YAML.stringify({ jobs: devcontainerJobs }, { lineWidth: 0 });
const devcontainerOutPath = join(ROOT, ".github", "workflows", "devcontainer-build.yml");
writeFileSync(devcontainerOutPath, header + devcontainerTemplate + "\n" + devcontainerJobsYaml, "utf8");
