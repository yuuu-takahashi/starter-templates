...
// rails-api-full: custom with redis + sidekiq
...
const RAILS_API_FULL_COMPOSE_SRC = join(
  ROOT,
  'shared',
  'docker',
  'docker-compose.rails-api-full.yml',
);
const NEXTJS_FULL_COMPOSE_SRC = join(
  ROOT,
  'shared',
  'docker',
  'docker-compose.nextjs-full.yml',
);
...
const YAML_HEADER_RAILS_API_FULL: string =
  '# Generated by scripts/generate-devcontainer.ts — edit shared/docker/docker-compose.rails-api-full.yml instead.\n';
const YAML_HEADER_NEXTJS_FULL: string =
  '# Generated by scripts/generate-devcontainer.ts — edit shared/docker/docker-compose.nextjs-full.yml instead.\n';
...
export function run(): void {
  ...
  const railsApiFullComposeContent = readFileSync(RAILS_API_FULL_COMPOSE_SRC, 'utf8');
  const railsApiFullComposeOut = join(
    ROOT,
    'full-templates/rails-api',
    '.devcontainer',
    'docker-compose.yml',
  );
  writeFileSync(
    railsApiFullComposeOut,
    YAML_HEADER_RAILS_API_FULL + railsApiFullComposeContent,
    'utf8',
  );
  logger.generated(railsApiFullComposeOut);

  const nextjsFullComposeContent = readFileSync(NEXTJS_FULL_COMPOSE_SRC, 'utf8');
  const nextjsFullComposeOut = join(
    ROOT,
    'full-templates/nextjs',
    '.devcontainer',
    'docker-compose.yml',
  );
  writeFileSync(
    nextjsFullComposeOut,
    YAML_HEADER_NEXTJS_FULL + nextjsFullComposeContent,
    'utf8',
  );
  logger.generated(nextjsFullComposeOut);
}
